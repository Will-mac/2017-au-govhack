...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.firebaseApp = firebase.initializeApp(this.authConfig);
  ...
  getRubbishDays() {
    fetch(`https://opencouncildata.cloudant.com/test1/_design/geo/_geo/garbage-collection-zones?lat=${this.state.data.userLat}&lon=${this.state.data.userLong}&radius=${0}&include_docs=true`)
      .then(response => response.json())
      .then(json => {
        this.setState({
          data: Object.assign({}, this.state.data, {
            rubbishStartTime: (new Date(json.rows[0].doc.properties.rub_start)).toISOString(),
            rubbishIntervalWeeks: json.rows[0].doc.properties.rub_weeks,
            recycleStartTime: (new Date(json.rows[0].doc.properties.rec_start)).toISOString(),
  ...
  updateNotifyTime = (e, notifyTime) => {
    this.setState({
      notifyTime,
      data: Object.assign({}, this.state.data, { notifyTime: notifyTime.toISOString() })
    });
  }
  ...
  signInOutView = () => {
    if (this.state.user) {
      ...
      return (
        <MenuItem onTouchTap={this.authenticate}>Sign In</MenuItem>
      );
    }
  }
    ...
    if (this.state.locationSaved) {
      return (
        <RaisedButton
          ...
          icon={<FontIcon className="material-icons">notifications_none</FontIcon>}
        />
      );
    } else {
      return (
    ...
    if (this.state.loading) {
      ...
      return (
        <div style={this.flexStyle}>
          ...
          <TimePicker defaultTime={this.state.notifyTime} onChange={this.updateNotifyTime}/>
          <div>
            <RaisedButton
      ...
      let difDate = Math.ceil((curDate - rubDate) / 3600 / 24 / 1000) % (7 * this.state.data.rubbishIntervalWeeks);
      console.log(difDate);
      return (
        <div style={this.flexStyle}>
